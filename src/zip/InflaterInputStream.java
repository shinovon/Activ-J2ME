...
public class InflaterInputStream extends InputStream {
	...
	public InflaterInputStream(InputStream in) {
		this(in, new Inflater(), BUF_SIZE);
	}

	/**
	 * Create an InflaterInputStream with the specified decompresseor and a default
	 * buffer size.
	 *
	 * @param in  the InputStream to read bytes from
	...
	protected void fill() throws IOException {
		if (in == null)
			throw new IOException("Zip: InflaterInputStream is closed");

		len = in.read(buf, 0, buf.length);

		if (len < 0)
			throw new IOException("Zip: Deflated stream ends early.");
	...
	public int read(byte[] b, int off, int len) throws IOException {
		...
		for (;;) {
			...
			try {
				...
				throw new IOException("Zip: " + dfe.getMessage());
			}
